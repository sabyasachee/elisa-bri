...
class OpinionMiner(nn.Module):
    ...
    def __init__(self, pretrained_model_name, max_holder_span_size, max_target_span_size, max_n_holders_per_sentence, max_n_targets_per_sentence):
        super().__init__()
        self.encoder = BertModel.from_pretrained(pretrained_model_name)
        
        self.max_holder_span_size = max_holder_span_size
        self.max_target_span_size = max_target_span_size
        self.max_span_size = max(self.max_holder_span_size, self.max_target_span_size)
        self.max_n_holders_per_sentence = max_n_holders_per_sentence
        self.max_n_targets_per_sentence = max_n_targets_per_sentence

        self.hidden_size = self.encoder.config.hidden_size
        self.conv1ds = []
        for i in range(self.max_span_size - 1):
            conv1d = nn.Conv1d(self.hidden_size, self.hidden_size, i + 2, bias=False)
            conv1d.weight.data = torch.zeros((self.hidden_size, self.hidden_size, i + 2))
        ...
        self.explicit_opinion_classifier = nn.Linear(self.hidden_size, 4)


    def forward(self, token_ids, mask, label_tuples=None):
        device = next(self.parameters()).device
        embedding = self.encoder(token_ids, mask)
        batch_size, seq_len, hidden_size = embedding.shape
        input = embedding.permute(0, 2, 1)

        span_representations = [embedding]
        for i in range(self.max_span_size - 1):
            output = self.conv1ds[i](input)
            output = torch.cat((output, torch.zeros((batch_size, hidden_size, i + 1), dtype=float, device=device)), dim=2)
            span_representations.append(output.permute(0, 2, 1))

        span_representations = torch.cat(span_representations, dim=1)
        ...
        holder_scores = torch.sigmoid(self.holder_scorer(span_representations).squeeze())
        target_scores = torch.sigmoid(self.target_scorer(span_representations).squeeze())

        holder_scores[holder_span_mask == 0] = -1
        target_scores[target_span_mask == 0] = -1

        holder_ranks = holder_scores.argsort(dim=1, descending=True)
        target_ranks = target_scores.argsort(dim=1, descending=True)

        implicit_input = torch.zeros((batch_size, self.max_n_targets_per_sentence, hidden_size), dtype=float, device=device)
        implicit_mask = torch.zeros((batch_size, self.max_n_targets_per_sentence), dtype=int, device=device)
        explicit_input = torch.zeros((batch_size, self.max_n_holders_per_sentence * self.max_n_targets_per_sentence, 2*hidden_size), dtype=float, device=device)
        ...
        for i in range(batch_size):
            ...
            for j in range(self.max_n_targets_per_sentence):
                ...
                if target_scores[i, r] > 0:
                    ...
                    target_to_label[target_key] = implicit_pred[i, j]
                ...
                if holder_scores[i, p] > 0:
                    ...
                    for k in range(self.max_n_targets_per_sentence):
                        ...
                        if target_scores[i, q] > 0:
                            ...
                            holder_and_target_to_label[key] = explicit_pred[i, j * self.max_n_targets_per_sentence + k]
        ...
        if label_tuples is not None:
            holder_label = torch.zeros((batch_size, seq_len * self.max_span_size), dtype=float, device=device)
            target_label = torch.zeros((batch_size, seq_len * self.max_span_size), dtype=float, device=device)
            ...
            implicit_label = torch.zeros((batch_size, self.max_n_targets_per_sentence, 4), dtype=torch.long, device=device)
            explicit_label = torch.zeros((batch_size, self.max_n_holders_per_sentence * self.max_n_targets_per_sentence, 4), dtype=torch.long, device=device)
            ...
            implicit_loss = cross_entropy_loss(implicit_logits, implicit_label)
            explicit_loss = cross_entropy_loss(explicit_logits, explicit_label)
